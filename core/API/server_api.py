...
class ServerAPI:
    ...
    def get_tweets_by_bbox(self, api_obj):
        ...
        valid_tweets = []
        _allowed_iterations = int(
            self._global_config['allowed_iterations'])
        _iterations = 0
        try:
            _couchConnector = CouchConnector(
                self._connector_config)
            _couchConnector.buffer_count = api_obj[
                'buffer_count']

            def buffering_without_continuation():
                tweets = _couchConnector.buffer_tweets_object(
                    api_obj['sin_key'])
                for tweet in tweets:
                    # print(tweet)
                    tweet_coords_arr = \
                    ...
                    if self.check_tweet_with_bbox(
                            tweet_coords_arr,
                            api_obj['region_bbox']):
                        valid_tweets.append(tweet)

            while len(valid_tweets) < api_obj[
                'minimum_tweets']:
    ...
    def get_tweets_by_bbox_buffer(self, api_obj,
                                  _couch_connector):
        try:
            print('get_tweets_by_bbox')
            valid_tweets = []
            ...
            def buffering_with_continuation():
                ...
                tweets = _couch_connector.buffer_tweets_object(
                    api_obj['sin_key'])
                for tweet in tweets:
                    # print(tweet)
                    tweet_coords_arr = \
            ...
            while len(valid_tweets) < api_obj[
                'minimum_tweets']:
                print('buffering_with_continuation->' + str(
                    len(valid_tweets)))
    ...
    def get_data_from_aurin_by_bbox(self, api_obj):
        ...
        valid_data = []
        try:
            _aurin_connector = AurinConnector(
                self._connector_config)

            data_obj = _aurin_connector.get_data(
                api_obj['dataset_name'])
            for feature in data_obj['features']:

                if 'boundedBy' in feature['properties']:
                    aurin_bbox = feature['properties'][
                        'boundedBy']
                    if self.aurin_data_with_bbox(aurin_bbox,
                                                 api_obj[
                                                     'region_bbox']):
                        valid_data.append(feature)

                elif 'geometry' in feature:
                    aurin_coor = feature['geometry'][
                        'coordinates']
                    # print(feature)
                    # print(aurin_coor)
                    if self.aurin_data_with_coords(
                            aurin_coor,
                            api_obj['region_bbox']):
                        valid_data.append(feature)

            return jsonify(
                {'data': valid_data, 'error': None})
    ...
    def get_customized_aurin_data_by_bbox_gluttony(self,
                                                   api_obj):
        print('get_customized_aurin_data_by_bbox_gluttony')
        try:
            _extract_aurin = ExtractAurin(
                self._connector_config)
            health_dict = _extract_aurin.getData(
                api_obj['dataset_name'])
            coordinates_dict = _extract_aurin.getCoordinatesData()
            # print(health_dict)
            # print(coordinates_dict)
            place_objects = formListPlaceObjects(
                dict(health_dict), dict(coordinates_dict))
            ...
            for place_object in place_objects:
                if self.aurin_data_with_bbox(
                        place_object.bounding_box,
                        api_obj['region_bbox']):
    ...
    def get_restaurants_data(self, api_obj):
        print('get_restaurants_data')
        try:
            config = configparser.ConfigParser()
            config.read(self._connector_config)
            data_path = config['melbournevic'][
                'restaurants_data']
            ret_data = {}
            #print(data_path)
            with open(data_path, encoding='ISO-8859-1') as csv_file:
                ...
                for row in csv_reader:
                    if line_count != 0:
                        obj = {}
                        if row[6] not in ret_data:
                            ret_data[row[6]] = [row[11],row[12]]
                    line_count += 1
            #print(ret_data)
            return jsonify(
                {'data': ret_data, 'error': None})
        except:
            traceback.print_exc()
    ...
    def check_tweet_with_bbox(self, tweet_coords_arr, bbox):
        ...
        if (x_coor >= bbox['xmin']) and (
                x_coor <= bbox['xmax']):
            if (y_coor >= bbox['ymin']) and (
                    y_coor <= bbox['ymax']):
                return True
        return False
    ...
    def aurin_data_with_bbox(self, aurin_bbox, bbox):
        ...
        if (aurin_xmin >= bbox['xmin']) and (
                aurin_xmax <= bbox['xmax']):
            if (aurin_ymin >= bbox['ymin']) and (
                    aurin_ymax <= bbox['ymax']):
                return True
        return False
    ...
    def aurin_data_with_coords(self, aurin_coors, bbox):
        try:
            ...
            try:
                ...
                traceback.print_exc()
                return False
        if (aurin_xcoor >= bbox['xmin']) and (
                aurin_xcoor <= bbox['xmax']):
            if (aurin_ycoor >= bbox['ymin']) and (
                    aurin_ycoor <= bbox['ymax']):
                return True
        return False
